             ...
             const lev_byte *strings[], size_t *symlistlen)
{
  ...
  symset = static_cast<short int*>(calloc(0x100, sizeof(short int)));
  if (!symset) {
    *symlistlen = (size_t)(-1);
    return NULL;
  }
  ...
  if (!symlist) {
    *medlength = 0;
    if (symlistlen != 0)
      ...
      return static_cast<lev_byte*>(calloc(1, sizeof(lev_byte)));
  }
  ...
  if (!symlist) {
    *medlength = 0;
    if (symlistlen != 0)
      ...
      return static_cast<wchar_t*>(calloc(1, sizeof(lev_wchar)));
  }
               ...
               size_t *medlength)
{
  ...
  *medlength = lengths[minidx];
  ...
  if (!result)
    return NULL;
  return static_cast<lev_byte*>(memcpy(result, strings[minidx], lengths[minidx]*sizeof(lev_byte)));
}
                 ...
                 size_t *medlength)
{
  ...
  *medlength = lengths[minidx];
  ...
  if (!result)
    return NULL;
  return static_cast<wchar_t*>(memcpy(result, strings[minidx], lengths[minidx]*sizeof(lev_wchar)));
}
/* }}} */
...
munkers_blackman(size_t n1, size_t n2, double *dists)
{
  ...
  /* allocate memory */
  covc = static_cast<size_t*>(calloc(n1, sizeof(size_t)));
  if (!covc)
    return NULL;
  zstarc = static_cast<size_t*>(calloc(n1, sizeof(size_t)));
  if (!zstarc) {
    free(covc);
    return NULL;
  }
  covr = static_cast<size_t*>(calloc(n2, sizeof(size_t)));
  if (!covr) {
    free(zstarc);
    free(covc);
    return NULL;
  }
  zstarr = static_cast<size_t*>(calloc(n2, sizeof(size_t)));
  if (!zstarr) {
    free(covr);
    free(zstarc);
    free(covc);
  ...
  zprimer = static_cast<size_t*>(calloc(n2, sizeof(size_t)));
  if (!zprimer) {
    free(zstarr);
    free(covr);
    free(zstarc);
...
lev_editops_invert(size_t n, LevEditOp *ops)
{
  ...
  for (i = n; i; i--, ops++) {
    ...
    if (op & 2)
      op ^= 1;
    ops->type=static_cast<LevEditType>(op);
/*
    if (ops->type & 2)
      ops->type ^= 1);
      */
                  ...
                  size_t *len)
{
  ...
  *len = dpos - dst;
  ...
  return static_cast<lev_byte*>(realloc(dst, *len*sizeof(lev_byte)));
}
                    ...
                    size_t *len)
{
  ...
  *len = dpos - dst;
  /* possible realloc failure is detected with *len != 0 */
  return static_cast<wchar_t*>(realloc(dst, *len*sizeof(lev_wchar)));
}
...
lev_opcodes_invert(size_t nb, LevOpCode *bops)
{
  ...
  for (i = nb; i; i--, bops++) {
    ...
    if (it & 2)
      it ^= 1;
    bops->type=static_cast<LevEditType>(it);
  }
}
